...
class Trainer:
    ...
    def forward_backward(
        ...
        response_masks: Int[torch.Tensor, "num_inputs max_seq_len"],
        loss_function,
    ):
        ...
        #~ Maybe model.train
        #~ do we need padding? 
        micro_bsz = self.training_args.micro_bsz
        ...
        micro_batches_masks: Int[torch.Tensor, "num_iterations micro_bsz max_seq_len"] = response_masks.view(num_iterations, micro_bsz, *response_masks.shape[1:])
        assert micro_batches.shape == micro_batches_masks.shape
        for i in range(num_iterations):
            ...
            mb_response_masks: Int[torch.Tensor, "micro_bsz max_seq_len"] = micro_batches_masks[i]
            logprobs: Float[torch.Tensor, "micro_bsz max_seq_len vocab_size"] = self.model(mb).logits
            loss = loss_function(mb, logprobs, old_logprobs, mb_advantages, mb_response_masks)
            loss.backward()
